...
int testBandwidth(unsigned int start, unsigned int end, unsigned int increment,
                   testMode mode, memcpyKind kind, printMode printmode, memoryMode memMode, int startDevice, int endDevice, bool wc);
int testBandwidthQuick(unsigned int size, memcpyKind kind, printMode printmode, memoryMode memMode, int startDevice, int endDevice, bool wc);
int testBandwidthRange(unsigned int start, unsigned int end, unsigned int increment,
...
*/
    if (dtod)
    {
        return testBandwidth((unsigned int)start, (unsigned int)end, (unsigned int)increment,
              ...
              testMode mode, memcpyKind kind, printMode printmode, memoryMode memMode, int startDevice, int endDevice, bool wc)
{
    ...
    {
        case QUICK_MODE:
            value = testBandwidthQuick(DEFAULT_SIZE, kind, printmode, memMode, startDevice, endDevice, wc);
            break;
...
{
    return testBandwidthRange(size, size, DEFAULT_INCREMENT, kind, printmode, memMode, startDevice, endDevice, wc);
}
                   ...
                   memcpyKind kind, printMode printmode, memoryMode memMode, int startDevice, int endDevice, bool wc)
{
    ...
    int return_bandwidths = (int)bandwidths[0];

    //clean up
    free(memSizes);
    free(bandwidths);

    return return_bandwidths;
}
